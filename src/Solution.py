...
def parse_hdr(hdr):
    ...
    submitter = hdr[15:20]
    sent_dt = hdr[26:34]
    return sysid,submitter,sent_dt

def parse_outbound_file(file):
    ORecord = collections.namedtuple(
        'ORecord',
        'sysid,submitter,sent_dt,,participant,participant_name,file_type,hdr'
    )
    sysid_ref={
        '46029': 'FAR',
        '46030' : 'COM'
    ...
    with open(file, 'r') as reader:
        ...
        for line in reader:
            if line[:3]=='HDR':
                sysid,submitter,sent_dt,=parse_hdr(line)
                hdr=line[:63]
...
def parse_confirm_file(file):
    CRecord = collections.namedtuple(
        'CRecord',
        'sysid,submitter,sent_dt,,status,hdr'
    )
    ...
    with open(file, 'r') as reader:
        ...
        for line in reader:
            if line[5:8]=='HDR':
                sysid,submitter,sent_dt,=parse_hdr(line[5:])
                hdr=line[5:68]
...
def dtcc_confirm():
    ...
    Dtcc_Confirm = collections.namedtuple(
        'Dtcc_Confirm',
        'sysid,submitter,sent_dt,participant,participant_name,file_type,status'
    )
    ...
    with open(output_path+'dtcc_outbound_status.csv', mode='w',newline='') as output_file:
        ...
        output_writer.writerow(['sysid','submitter','sent_dt','participant','participant_name','file_type','status'])
